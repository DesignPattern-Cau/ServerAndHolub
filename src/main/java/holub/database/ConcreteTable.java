 ...
 *    in any of this code.
 */
package holub.database;

import java.io.*;
import java.util.*;

import holub.database.jdbc.ColumnVisitor;
import holub.database.jdbc.Visitor;
...
/* package */ public class ConcreteTable implements Table {
	...
	public void accept(Visitor visitor){
		visitor.visit(this.columnNames);
	}
	...
	public Table select(Selector where, String[] requestedColumns, // {=ConcreteTable.select.default}
		...
		//TODO : RequestColumns를 compute할 수 있다면 문제 해결 가능!
		if(requestedColumns == null){
			//load all
			Visitor columnSelector = new ColumnVisitor();
			for (Table allTable : allTables) {
				allTable.accept(columnSelector);
			}
			requestedColumns = columnSelector.result();
		}
	...
	public Table select(Selector where, Collection requestedColumns, Collection other) {
		...
		//Join target tables -> *시 애내의 field도 따오는지 확인해야함!
	...
	public final static class Test {
		...
		public void test() {
			try {
				...
				System.out.println("////////////////////////////////");
				testSelect();
				Writer out = new FileWriter( "address.csv" );
				address.export( new CSVExporter(out));
				out.close();
			} catch (Throwable t) {
				report(t, "Insert");
			}
...
//				/* Failed correctly, do nothing */ }
		...
		public void testSelect() {
			Selector flintstoneSelector = new Selector.Adapter() {
				public boolean approve(Cursor[] tables) {
					return tables[0].column("last").equals("Holub");
				}
			};
